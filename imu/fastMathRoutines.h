...
inline float Rajan_FastArcTan(float x) {
  return PI_F/4.0*x - x*(fabs(x) - 1)*(0.2447 + 0.0663*fabs(x));
}
...
inline float Rajan_FastArcTan2(float y, float x) {
  ...
  if (swap45) {
    ...
    switch (qCode) {    
      case 0: z = z;         break;
      case 1: z = PI_F + z;    break;
      case 2: z = -PI_F + z;   break;
      case 3: z = z;         break;
    }
  }
...
int32_t Rajan_FastArcTan2_deg1000(float y, float x) {
  return 180/PI_F * 1000 * Rajan_FastArcTan2(y, x);
}
