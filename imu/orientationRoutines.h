/**********************************************
*
*	IMU orientation control for PIC24 Olin College board
*
...
#define MPU6050_GYRO_FS 0x00 //init to 250deg/s
...
void gyroOffsetCalibration()
{
  ...
  while(calibGCounter>0)
  {
    ...
    {
      __delay_ms(700); // wait 0.7sec if calibration failed
      gyro_get_measurements(&gyro[0], &gyro[1], &gyro[2]); 
      for (i=0; i<3; i++) {
        fp_gyroOffset[i] = 0;
        prevGyro[i]=gyro[i];
      }
    ...
    if(tiltDetected>=1)
    {
      printf("Motion detected during Gyro calibration. Starting over!\n");
