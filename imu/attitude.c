/**********************************************
*
*	attitude estimation for PIC24 Olin College board
*
*	Adapted from the BruGi project, code.google.com/p/brushless-gimbal
*	This implements a simplified complimentary filter based on Kalman filtering
*
...
void readGyros() {
  ...
  // read gyros
  gyro_get_measurements(&axisRot[0], &axisRot[1], &axisRot[2]);
...
void readACC(axisDef axis) {
  ...
  switch(idx) {
	case 0: val = (((int16_t)accel_read(XOUT8)) << 8);
		break;
	case 1: val = (((int16_t)accel_read(YOUT8)) << 8);
		break;
	case 2: val = (((int16_t)accel_read(ZOUT8)) << 8);
		break;
	default: break;
  }
